...
## INPUT PARAMETERS
nconf =   50 # number of configurations to average over
iskip =   949 #number of configurations to skip before taking data
...
for kconf in range(istart,nconf+iskip+1): 
    ...
    for j in range(1,dim):
        ...
        if zc[k] <= zbox2:             #combine the two layers of lamellae
            zc1[k] = zc[k]
        elif zc[k] > zbox2 and zc[k] <= 2*zbox2:
            zc1[k] = zc[k] - zbox2
        theta[k] = (2*pi/zbox2)*zc1[k] #circling z-axis for one layer (combined with others)
        cirx[k] = (zbox2/(2*pi))*sin(theta[k]) #x coordinate in cylindrical coordinates(xytheta), R=zbox/(2*pi)
        ciry[k] = (zbox2/(2*pi))*cos(theta[k]) #y coordinate in cylindrical coordinates(xytheta), R=zbox/(2*pi)        
        if typea[k] == 1:
